...
# !pip install -U --user git+https://github.com/graphistry/cu-cat.git@DT
...
states = ["CA", "CO", "CT", "DC", "DE", "FL", "GA", "HI", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD", "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ", "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY"]
for s in states:
    ddf=dc.read_csv("../../data-pv/ddf_gby/*")
    ...
    for d in pd.unique(filtered_state_df.DRUG_NAME):
        ...
        t=time()
        g1=g.featurize(feature_engine='cu_cat',memoize=True)
        ...
        t=time()
        # g3 = g.umap(feature_engine='dirty_cat', engine='umap_learn',memoize=False)
        t3=time()-t
        
        df2 = pd.DataFrame([[s,d,t1,t3,rows.shape]], columns=['state','drug','gpu','dc-cuml','dims'], index=[d+'_'+s])
        print(df2)
        A=A.append(df2)
        df2.to_csv('gpu_feat_loop_bench.txt',sep='\t',mode='a')
    del g1, ddf,filtered_state_df,state_df
A.to_csv('gpu_full_bench2.txt',sep='\t')